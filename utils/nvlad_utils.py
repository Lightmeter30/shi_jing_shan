...
def setup_directories(req_loc, tempfolder):
    ...
    if not os.path.exists(tempfolder):
        ...
        os.makedirs(os.path.join(tempfolder, 'debug'))
...
depths: 有效点深度 N
'''
def pixel_to_world(kpoints1, depth_image, K1, P_c2w_3DS, Z_Far):
    """Process a single image for matching."""
    # 1. 检查深度图
    ...
    if len(depths) > 0:
        ...
        min_depth_point = points_valid[min_depth_idx]
        max_depth_point = points_valid[max_depth_idx]
        print(f'  - Mininum pixel depth: {np.min(depth_values[valid_mask])} at pixel ({min_depth_point[1]}, {min_depth_point[0]})')
        print(f'  - Maximum pixel depth: {np.max(depth_values[valid_mask])} at pixel ({max_depth_point[1]}, {max_depth_point[0]})')
    ...
    # points_3DS = transfer_Pixel_from_CV2_to_3DS(points_valid)
    # points_3DS = points_valid
    camera_3dpoints_3DS = np.zeros((len(points_cv2), 4))
    camera_3dpoints_3DS[:, 0] = (points_cv2[:, 1] - c_y) * depths / f_y
    camera_3dpoints_3DS[:, 1] = (points_cv2[:, 0] - c_x) * depths / f_x
    # camera_3dpoints_unity[:, 1] = -(points_valid[:, 1] - c_y) * depths / f_y
    ...
    # world_coords = (Rt.T @ camera_coords.T).T - (Rt.T @ t).reshape(1, 3)
    # # 转换为齐次坐标
    # world_coords = np.hstack((world_coords, np.ones((world_coords.shape[0], 1))))
    # print(world_coords.shape)
    
    # world_coords = (P_inv[:3,:3] @ camera_coords[:,:3].T).T
    model_3dpoints = (P_c2w_3DS @ camera_3dpoints_3DS.T).T
    # world_3dpoints_cv2 = transfer_Point_from_3DS_to_CV2(model_3dpoints)
    model_3dpoints = cv2.convertPointsFromHomogeneous(model_3dpoints).squeeze()
    # temp = (P @ world_coords.T).T / depths.reshape(-1, 1)
    

    # 11. 记录被移除点的索引
    remove_index_list = np.where(~valid_mask)[0].tolist()
    
    # print(f'the number of feature points which have a legal depth: {len(world_3dpoints_cv2)}')
    
    return [model_3dpoints, remove_index_list, camera_3dpoints_3DS, points_cv2, depths]
...
inliners: 有效点索引
'''
def estimate_pose_PNPRANSAC(points3d_3DS, kpoints3, K3, P1_c2w_3DS, distCoeffs=None):
    ...
    if success and inliners is not None:
        ...
        print(f'pose_c2w_3DS: {pose_c2w_3DS}')
...
def debug_evaluate_pnp_pose(points_3d, points_2d, K, pose_matrix, output_path):
    """
    直接评估PnP求解的位姿
    
    参数:
        points_3d: 已知的3D点 (N, 3)
        points_2d: 对应的2D图像点 (N, 2) 
        K: 相机内参
        pose_matrix: PnP求解的位姿矩阵 c2w
    """
    # 构建投影矩阵
    T = transfer_Pose_from_3DS_to_CV2(pose_matrix)
    T = invert_Pose_Matrix(T)
    P = K @ T[:3, :]
    ...
    proj_2d_h = (P @ points_3d_h.T).T
    proj_2d = (proj_2d_h[:, :2].T / proj_2d_h[:, 2]).T
    
    # 计算重投影误差
    reprojection_errors = np.linalg.norm(proj_2d - points_2d_new, axis=1)
    # 保存重投影误差
    with open(os.path.join(output_path, 'reprojection_errors.txt'), 'w') as f:
        f.write(f'mean: {np.mean(reprojection_errors)}\n')
        f.write(f'median: {np.median(reprojection_errors)}\n')
        f.write(f'max: {np.max(reprojection_errors)}\n')
        f.write(f'min: {np.min(reprojection_errors)}\n')
        for i in range(reprojection_errors.shape[0]):
            f.write(f'{reprojection_errors[i]}\n')
