...
def exifori_to_unity_rotation_matrix(image_path):
    exif_data = piexif.load(image_path)["0th"]
    if 274 not in exif_data:
        return np.identity(4)
    exif_orientation = exif_data[274]  # 或者直接通过参数传递exif_orientation
    valid_orientation = (0, 3, 6, 8)
    assert exif_orientation in valid_orientation
    rotation_deg_z = get_rotation_deg_z(exif_orientation)
    return get_unity_rotation_matrix(rotation_deg_z)

if __name__ == '__main__':
    image_path = '/home/tcluan/0-Desktop/gxl.jpg'
